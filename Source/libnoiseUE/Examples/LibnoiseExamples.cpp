...
void ULibnoiseExamples::CreateTerrainHeightMapGrayscale()
{
	...
	FString ImageFilePath = FPaths::ProjectDir() + "/HeightMapGrayscale.bmp";

	utils::WriterBMP Writer;
	Writer.SetSourceImage(Image);
	Writer.SetDestFilename(std::string(TCHAR_TO_UTF8(*ImageFilePath)));
...
void ULibnoiseExamples::CreateSphericalPlanetaryTerrain()
{
	...
	FString ImageFilePath = FPaths::ProjectDir() + "/SphericalPlanetaryTerrain.bmp";
...
void ULibnoiseExamples::CreateComplexPlanet()
{
	...
	if (resInMeters <= 240.0) {
		FString ImageFilePath = FPaths::ProjectDir() + "/terrain.ter";
		
		utils::WriterTER terrainWriter;
		terrainWriter.SetSourceNoiseMap(elevGrid);
		terrainWriter.SetDestFilename(std::string(TCHAR_TO_UTF8(*ImageFilePath)));
	...
	std::ofstream os;
	os.open(std::string(TCHAR_TO_UTF8(*ImageFilePath)), std::ios::out | std::ios::binary);
	...
	ImageFilePath = FPaths::ProjectDir() + "/terrain.bmp";

	// Write the image as a Windows bitmap file (*.bmp).
	utils::WriterBMP bitmapWriter;
	bitmapWriter.SetSourceImage(destImage);
	bitmapWriter.SetDestFilename(std::string(TCHAR_TO_UTF8(*ImageFilePath)));
	...
	ImageFilePath = FPaths::ProjectDir() + "/terrainsurface.bmp";
	...
	ImageFilePath = FPaths::ProjectDir() + "/terrainspec.bmp";
	...
	ImageFilePath = FPaths::ProjectDir() + "/terrainnormal.bmp";
...
void ULibnoiseExamples::CreateCloudsOverWater()
{
	...
	FString ImageFilePath = FPaths::ProjectDir() + "/textureplane.bmp";
	...
	CreatePlanarTexture(finalWater, finalClouds, false, TEXTURE_HEIGHT,
		TCHAR_TO_UTF8(*ImageFilePath));

	ImageFilePath = FPaths::ProjectDir() + "/textureseamless.bmp";
